...
const AssignProjectTeam = () => {
  ...
  useEffect(() => {
    ...
    const fetchStandardTasks = async () => {
      try {
        const managersResponse = await fetchStandardTaskByProjectTeam(
          "Project Manager"
        );
        setProjectManagerTasks(managersResponse?.data);

        const supervisorsResponse = await fetchStandardTaskByProjectTeam(
          "Project Supervisor"
        );
        setProjectSupervisorTasks(supervisorsResponse?.data);

        const coordinatorsResponse = await fetchStandardTaskByProjectTeam(
          "Site Coordinator"
        );
    ...
    const fetchProjectTeamId = async () => {
      const projectManagerResp = await fetchProjectTeamIdByUserId(
        project_manager
      );
      setProjectManagerTeamId(projectManagerResp?.data[0]?.id);

      const projectSupervisorResp = await fetchProjectTeamIdByUserId(
        project_supervisor
      );
          ...
          <View style={styles.taskBlock}>
            <View style={styles.section}>
              {projectSupervisor ? (
                ...
                <Text style={styles.loadingText}>
                  Loading Project Supervisor details...
                </Text>
              )}
            </View>
            <View style={styles.section}>
          ...
          <View style={styles.taskBlock}>
            <View style={styles.section}>
              {siteCoordinator ? (
                ...
                <Text style={styles.loadingText}>
                  Loading Site Coordinator details...
                </Text>
              )}
            </View>
            <View style={styles.section}>
