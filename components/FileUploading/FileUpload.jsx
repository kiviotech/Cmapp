    ...
    const handleImageSelected = async (result) => {
      try {
        if (!result.canceled && result.assets?.length > 0) {
          // Clear any existing errors when files are selected
          if (setErrors) {
            setErrors("");
          }
    ...
    const handleRemoveFile = useCallback(
      (fileName) => {
        ...
        setUploadedFiles((prev) => {
          ...
          if (removedFile?.fileId) {
            setFileIds((prevIds) => {
              ...
              setTimeout(() => {
                ...
                // Set error if no files remain
                if (setErrors && newIds.length === 0) {
          ...
          // If this was the last file, set the error
          if (remainingFiles.length === 0 && setErrors) {
            setErrors("Please upload at least one file");
          }
